...
export type Department = { id: string; organizationId: string; branchId: string; branchName: string; code: string; name: string; status: "ACTIVE" | "INACTIVE" };

async function context() {
  const c = await getOrganizationContext();
  if (!c) throw new Error("UNAUTHENTICATED");
  requireOrganizationPermission(c, "department.manage");
  return c;
}
...
export async function listDepartments() {
  ...
  const { data, error } = await createAdminClient()
    .from("departments")
    .select("id,organization_id,branch_id,code,name,status,branches!inner(name)")
  ...
  return (data ?? []).map((d: any) => ({
    ...
    branchName: Array.isArray(d.branches) ? d.branches[0]?.name ?? "" : d.branches?.name ?? "",
    code: d.code,
    name: d.name,
    status: d.status,
  })) as Department[];
...
export async function createDepartment(input: Omit<Department, "id" | "organizationId" | "branchName">) {
  const c = await context();
